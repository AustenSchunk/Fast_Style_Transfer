...
class LookupAlignConvolution2d(Conv2D):
    ...
    def build(self, input_shape):
        ...
        self._convolution_op = nn_ops.Convolution( input_shape, filter_shape=self.kernel.get_shape(),
                                        dilation_rate=self.dilation_rate, strides=self.strides, padding=self.padding.upper(),
...
def lookup_conv2d(tensor_in, num_outputs, kernel_size, stride, dict_size, padding=1,
    ...
    in_height = tf.to_float(in_shape[1])
    in_width = tf.to_float(in_shape[2])

    out_height = tf.math.ceil(tf.to_float(in_height) / tf.to_float(stride[0]))
    out_width  = tf.math.ceil(tf.to_float(in_width) / tf.to_float(stride[0]))

    pad_along_height = tf.math.maximum((out_height - 1) * stride[0] +
                        kernel_size[0] - in_height, 0)
    pad_along_width = tf.math.maximum((out_width - 1) * stride[0] +
                       kernel_size[0] - in_width, 0)
    ...
    padded = tf.pad(tensor_in, [[0, 0], [pad_top, pad_bottom], [pad_left, pad_right], [0, 0]], "CONSTANT")
    # padded = tf.pad(tensor_in, [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT")
    ...
    if gen_sparse_conv:
        ...
        if mode == 'tf_op':
            ...
            output = sparse_conv2d_m.sparse_conv2d(pool_conv, dense_kernel_idx, dense_kernel_val, dense_shape=dense_kernel_shp, strides=stride)
