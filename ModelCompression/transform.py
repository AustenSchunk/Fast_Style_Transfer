"""
Contains method to perform image transformation
"""
...
def net(image):
    ...
    as in Johnson et al. paper but applies thresholded pruning to the weights
...
def pruned_conv_layer(net, num_filters, filter_size, strides, layerNum, relu=True):
    ...
    Args:
        ...
        filter_size: filter size, i.e. (filter_size x filter_size)
        strides: size of strides for each layer
        layerNum: layer number, used mainly for tensorboard visualization
        relu: boolean of whether or not to add a relu activation

    Returns:
        4D tensor transformed by this layer
    """
    with tf.variable_scope('conv{0}'.format(layerNum)) as scope:

        kernel = pruned_conv_weights_init(net, num_filters, filter_size)
        strides_shape = [1, strides, strides, 1]
        # line pruning.apply_mask is what implements the tensorflow pruning ops
...
def pruned_conv_tranpose_layer(net, num_filters, filter_size, strides, layerNum):
    """
    Performs convolution transpose with application of pruned weights
...
def pruned_conv_weights_init(net, out_channels, filter_size, transpose=False):
    ...
    Returns:
        Weights with pruning operations added
    """
...
def _residual_block(net, layerNum, filter_size=3):
    """
    Application of a residual convolutional block
    ...
    Returns:
        4D tensor transformed by residual block
    """
